...
| 能力 | 说明 |
|------|------|
| 🧠 **多模型公平对决** | DeepSeek / GLM / Claude / GPT 等任意 OpenAI 兼容模型,同数据、同工具、同起点资金竞技;`configs/*.json` 加一条即可接入,无需改代码 |
| 🌐 **多市场模拟盘** | 美股 / A股 / 港股同时回放竞技,`backend.yaml` 加一条即可扩展任意市场 |
| 💰 **A股实盘** | 通达信桥 8550 实盘下单,持仓/成交/净值全链路,分账制子账户(每 agent 独立额度) |
| ⏰ **盘中智能调度** | 开盘 + 每小时定时 + 波动触发(持仓盈亏 ±3pp / 个股涨跌 ≥5% 立即加跑),LLM 逐只简评 |
| 🎯 **交易思路可定制** | 分析模式(基线/苦行/博弈…)+ 风控参数 + 交易记忆,三层自由定义,无需改代码 |
| 🛡️ **风控网关** | 单笔/持仓限额、日亏熔断、现金保留、黑名单,三条交易路径单点拦截 |
| 📝 **交易记忆** | 开盘读心得、收盘写经验,超 200 行自动归档,策略越用越好用 |
| 🐳 **Docker 一键部署** | compose 编排全部服务,前端多阶段自动构建,克隆即跑(含 AI 部署 runbook) |
...
- [x] 多市场模拟盘(US/CN/HK) + 多模型对决 + Arena 可视化
- [x] A股实盘(通达信桥) + 分账制 + L2 微观因子
- [ ] 更多券商接入验证(富途/老虎/IB 已预留 adapter)
- [ ] 交易记忆跨市场迁移 / 因子自动挖掘联动(RD-Agent)
