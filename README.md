...
| 🎯 **交易思路可定制** | 三层:①分析模式(`prompts/analysis_modes.py` 预设基线/苦行/博弈等,前端比赛配置按模型勾选,加模式即改 prompt)②风控参数(`backend.yaml` → risk:单笔/持仓限额、日亏熔断、黑名单)③交易记忆沉淀 + dsh skills(自定义分析流程),交易思路完全由你定义 |
...
| **QuantDB 数据底座** | QuantMind 内置的付费 CDN 量化数据源,按数据集同步到本地 parquet;本仓库已预留只读挂载点 `/data/quantdb`,未订阅时自动降级(免费接口兜底,随时平滑升级) | 见 QuantMind 仓库 |
