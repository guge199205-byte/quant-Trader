...
def buy(symbol: str, amount: int) -> Dict[str, Any]:
    ...
    market = "cn" if symbol.endswith((".SH", ".SZ")) else ("hk" if symbol.endswith(".HK") else "us")
    ...
    # 成交价含滑点（买 +slippage），现金扣款含手续费
    from agent_tools.risk import get_trading_config

    _fees = get_trading_config()
    _exec_price = float(this_symbol_price) * (1 + _fees["slippage"])
    _fee = _exec_price * amount * _fees["buy_rate"]
    try:
        cash_left = float(current_position["CASH"]) - _exec_price * amount - _fee
    except (KeyError, TypeError, ValueError) as e:
        return {
            "error": f"Failed to compute cash balance: {e}",
            "symbol": symbol,
            "date": today_date,
        }
    ...
    # 风控网关：单笔/持仓/熔断/黑名单校验
    from agent_tools.risk import pre_trade_check

    risk_error = pre_trade_check(
        "buy", symbol, amount, _exec_price, current_position, signature
    )
    if risk_error:
        return {"error": risk_error, "symbol": symbol, "date": today_date}
...
def sell(symbol: str, amount: int) -> Dict[str, Any]:
    ...
    risk_error = pre_trade_check(
        "sell", symbol, amount, 0.0, current_position, signature
    )
    if risk_error:
        return {"error": risk_error, "symbol": symbol, "date": today_date}
    ...
    # Increase cash balance: 成交价含滑点（卖 -slippage），收入扣卖出手续费
    from agent_tools.risk import get_trading_config

    _fees = get_trading_config()
    _exec_price = float(this_symbol_price) * (1 - _fees["slippage"])
    _fee = _exec_price * amount * _fees["sell_rate"]
    new_position["CASH"] = new_position.get("CASH", 0) + _exec_price * amount - _fee
