...
class DataLoader {
    ...
    getMarketConfig() {
        if (window.configLoader && window.configLoader.getMarketConfig) {
    ...
    async loadAgentList() {
        try {
            ...
            const agents = [];
            const enabledAgents = window.configLoader && window.configLoader.getEnabledAgents 
                ? window.configLoader.getEnabledAgents(this.currentMarket) 
                : [];

            // If no enabled agents found, return empty array
            if (enabledAgents.length === 0) {
                console.log('No enabled agents found for market:', this.currentMarket);
                return [];
            }
    ...
    // Load all market stock prices from merged.jsonl (cn -> A_stock, hk -> HK_stock)
    async loadAStockPrices() {
        ...
        try {
            const priceFile = this.currentMarket === 'hk'
                ? 'HK_stock/merged.jsonl'
                : 'A_stock/merged.jsonl';
            const response = await fetch(`${this.baseDataPath}/${priceFile}`);
            if (!response.ok) throw new Error(`Failed to load prices from ${priceFile}`);
            ...
            console.log(`Loaded prices for ${Object.keys(this.priceCache).length} stocks from ${priceFile}`);
            return this.priceCache;
        } catch (error) {
    ...
    async loadStockPrice(symbol) {
        ...
        if (['cn', 'hk'].includes(this.currentMarket)) {
    ...
    async calculateAssetValue(position, date) {
        ...
        if (['cn', 'hk', 'us'].includes(this.currentMarket) && hasMissingPrice) {
            return null;
        }
    ...
    async loadBenchmarkData() {
        ...
        if (this.currentMarket === 'us') {
            return await this.loadQQQData();
        } else if (['cn', 'hk', 'us'].includes(this.currentMarket)) {
    ...
    async loadAllAgentsData() {
        ...
        try {
            ...
            // If no agents found, return empty object
            if (agents.length === 0) {
                console.log('No agents found for market:', this.currentMarket);
                return {};
            }
