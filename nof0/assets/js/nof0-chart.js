...
class Nof0ChartManager {
    ...
    prepareDatasets() {
        ...
        Object.entries(this.agentsData).forEach(([agentName, agentData]) => {
            ...
            datasets.push({
                ...
                tension: 0.38, // 更顺滑的曲线
                borderWidth: isSelected ? 3.5 : 1.5, // 选中时线条更粗
                pointRadius: isSelected ? 0 : 0, // 未选中不显示点
                pointHoverRadius: isSelected ? 9 : 6,
        ...
        Object.keys(this.agentsData).forEach(agentName => {
            ...
            legendItem.innerHTML = `
                <div class="legend-chip-icon" style="background-color: ${color}20; border: 1px solid ${color}40;">
                    <img src="${this.iconPaths[agentName] || './figs/stock.svg'}" alt="${displayName}" onerror="this.style.display='none'">
                </div>
    ...
    calculateYAxisRange() {
        ...
        // 波动小时用固定下限，避免近平线被 10% padding 压成大片填充区
        const padding = Math.max(range * 0.05, range < 1 ? 0.002 * maxValue : 0);
