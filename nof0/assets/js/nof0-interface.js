...
class Nof0Interface {
    ...
    setupEventListeners() {
        ...
        const hkMarketBtn = document.getElementById('hkMarketBtn');
        if (hkMarketBtn) {
            hkMarketBtn.addEventListener('click', () => {
                this.switchMarket('hk');
            });
        }
    ...
    switchMarket(market) {
        ...
        this.loadStockNames(); // 切换市场重新加载名称表
    ...
    async loadStockNames() {
        try {
            if (window.configLoader && window.configLoader.getDataPath) {
                const base = window.configLoader.getDataPath().replace(/\/api\/data$/, '/api');
                const token = window.configLoader.getApiToken ? window.configLoader.getApiToken() : '';
                const headers = token ? { 'X-API-Token': token } : {};
                const resp = await fetch(`${base}/stock-names?market=${this.currentMarket}`, { headers });
                const data = await resp.json();
                this.stockNames = data.data || {};
            }
        } catch (e) { /* 名称表加载失败不影响功能 */ }
    }
    ...
    createPositionItem(agentName, symbol, shares) {
        ...
        // 股票中文名（从 API 获取的映射表）
        const displaySymbol = (this.stockNames && this.stockNames[symbol]) || symbol;
        ...
        // 货币符号按当前市场对齐（US: $ / CN: ¥）
        const currencySymbol = this.currentMarket === 'cn' ? '¥' : '$';
        ...
        item.innerHTML = `
            <div class="position-header">
                ...
                <div class="position-shares">${shares} 股</div>
            </div>
            <div class="position-footer">
                <div class="position-agent">${agentDisplayName}</div>
                <div class="position-value">${currencySymbol}${(shares * 100).toFixed(2)}</div>
            </div>
        `;
