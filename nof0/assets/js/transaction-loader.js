...
class TransactionLoader {
    ...
    async loadAllTransactions() {
        ...
        // Ensure dataLoader is available and has getMarket method
        if (!dataLoader || typeof dataLoader.getMarket !== 'function') {
            console.warn('[TransactionLoader] DataLoader not available or missing getMarket method');
            return [];
        }
    ...
    async loadAgentTransactions(agentFolder, market = 'us') {
        try {
            ...
            const baseData = window.configLoader.getDataPath();
            const positionPath = `${baseData}/${agentDataDir}/${agentFolder}/position/position.jsonl`;
    ...
    async loadAgentThinking(agentFolder, date, market = 'us') {
        try {
            ...
            const agentDataDir = marketConfig ? marketConfig.data_dir : 'agent_data';
            const baseData = window.configLoader.getDataPath();
            const logPath = `${baseData}/${agentDataDir}/${agentFolder}/log/${date}/log.jsonl`;
