...
for D in 2026-08-25 2026-08-26 2026-08-27; do
  echo "==== 补跑 $D ===="
  docker compose --profile agents run --rm -e "INIT_DATE=$D" -e "END_DATE=$D" agent-us
...
# 3) 恢复 08-28 行（追加文件尾；get_latest_position 按当天/前一个交易日过滤，不受顺序影响）
cat "/tmp/us_pos_0828_${TS}.jsonl" >> "$POS"
