#!/usr/bin/env python3
"""港股分账账本（per-agent，独立于 A股 live_ledger.json）。

- data/hk_ledger.json：{agents: {name: {quota, virtual_cash, positions: {code: {...}}}}}
- 初始虚拟现金 HK$10 万/agent（hk_config.json initial_cash 可配）
- 买入扣虚拟现金、卖出加回；持仓成本加权平均
- 与 A股同一套不可变记账语义（record_buy/record_sell 返回新账本）
"""
...
def agent_virtual_cash(ledger: dict, agent: str) -> float:
    """该 agent 虚拟现金（初始 HK$10 万；买入扣、卖出加）。"""
