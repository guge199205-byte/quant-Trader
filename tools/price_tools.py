...
def get_merged_file_path(market: str = "us") -> Path:
    ...
    if market == "cn":
        ...
        return base_dir / "data" / "HK_stock" / "merged.jsonl"
    else:
        return base_dir / "data" / "merged.jsonl"


def _iter_time_series_keys(data: dict):
    """Yield time-series key -> value maps from a merged.jsonl doc.

    Handles both daily ("Time Series (Daily)") and intraday/hourly
    ("Time Series (60min)", ...) formats by matching the "Time Series" prefix.
    """
...
def _ts_key_matches_date(ts_key: str, date: str) -> bool:
    """Match a time-series key against a date, with or without hour component."""
    return ts_key == date or ts_key.startswith(date + " ")
        ...
        with open(merged_file_path, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    ...
                    for ts in _iter_time_series_keys(data):
                        for ts_key in ts:
                            if _ts_key_matches_date(ts_key, date):
                                return True
        ...
        with open(merged_file_path, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    data = json.loads(line.strip())
                    for ts in _iter_time_series_keys(data):
                        # Normalize hourly keys to their date part
                        for ts_key in ts:
                            trading_days.add(ts_key.split(" ")[0])
                except json.JSONDecodeError:
                    continue
    ...
    with merged_file.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if not isinstance(series, dict):
                continue
            # 兼容日级（精确 key）与小时级（日期前缀匹配）两种格式
            bar = series.get(today_date)
            if not isinstance(bar, dict):
                date_prefix = today_date[:10]
                for ts_key, candidate in series.items():
                    if ts_key.startswith(date_prefix) and isinstance(candidate, dict):
                        bar = candidate
                        break
